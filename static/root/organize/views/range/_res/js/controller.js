...
app.controller('rangeCtrl',function ($scope,$state) {
    ...
    $scope.delete = function(){
        if($scope.id){
            $state.go('root.organize.views.range.list[12]',{id:$scope.id,name:'delete',page:$scope.page});
            $scope.menuClass = 'deleteMenu';
        }
    };
    $scope.close = function(){
        if($scope.id){
            $state.go('root.organize.views.range.list[12]',{id:$scope.id,name:'close'});
            $scope.menuClass = 'closeMenu';
        }
    };
    $scope.reset = function(){
        if($scope.id){
            $state.go('root.organize.views.range.list[12]',{id:$scope.id,name:'reset'});
            $scope.menuClass = 'resetMenu';
        }
    };
    $scope.edit = function(){
        if($scope.id){
            $state.go('root.organize.views.range.edit[12]',{id:$scope.id});
